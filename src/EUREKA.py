...
class surface:
    def __init__(self,
        ...
        self.going=None
        self.fan=0
        self.light_edge = (np.random.rand(),np.random.rand(),np.random.rand())
...
def are_points_numerically_identical(point1, point2, tolerance=1e-9):
    """
    Checks if two 3D points (lists/arrays of floats) are numerically identical
    within a given tolerance.
    """
    if len(point1) != 3 or len(point2) != 3:
        return False # Ensure they are 3D points
    for i in range(3):
        if math.fabs(point1[i] - point2[i]) > tolerance:
            return False
    return True

# --- Function to add unique points ---
def add_unique_point(unique_points_list, new_point, tolerance=1e-9):
    """
    Adds a new_point to unique_points_list if it's not already
    numerically present.
    Returns True if added, False otherwise.
    """
    # Convert to float list for consistent comparison
    new_point_float = [float(x) for x in new_point]

    for existing_point in unique_points_list:
        if are_points_numerically_identical(existing_point, new_point_float, tolerance):
            return False # Duplicate found

    unique_points_list.append(new_point_float)
    return True
...
def plot_surface(ax: Axes3D, sur: surface, normal=False):
    ...
    try:
        ...
        lines = ["surface;position;normal;corners\n"]
    with open("testcases.txt",'w') as writer:
        ...
        for edge in edges:
            ...
            ys = [sta[1], end[1]]
            zs = [sta[2], end[2]]
            add_unique_point(all_unique_points, list(sta))  # Convert numpy array back to list for consistency
            add_unique_point(all_unique_points, list(end))
        ...
        formatted_unique_points = [fmt(p) for p in all_unique_points]
        writer.write(f"{sur.sid}; {sur.vertex.tolist()}; {sur.normal.tolist()};{formatted_unique_points}\n")
